...
#include "transsion_exchange_connector.h"
...
namespace {
Logging::Category transsionExchangeConnectorTrace("Transsion Exchange Connector");
Logging::Category transsionExchangeConnectorError("[ERROR] Transsion Exchange Connector", transsionExchangeConnectorTrace);
}

namespace RTBKIT {

/*****************************************************************************/
/* TRANSSION EXCHANGE CONNECTOR                                                */
/*****************************************************************************/

TranssionExchangeConnector::
TranssionExchangeConnector(ServiceBase & owner, const std::string & name)
...
TranssionExchangeConnector::
TranssionExchangeConnector(const std::string & name,
...
getCampaignCompatibility(const AgentConfig & config,
    ...
    const Json::Value & pconf = config.providerConfig["transsion"];

    try {
        cpinfo->seat = Id(pconf["seat"].asString());
        if (!cpinfo->seat)
            result.setIncompatible("providerConfig.transsion.seat is null",
                                   includeReasons);
    } catch (const std::exception & exc) {
        result.setIncompatible
        (string("providerConfig.transsion.seat parsing error: ")
    ...
    try {
        ...
        if (!cpinfo->iurl.size())
            result.setIncompatible("providerConfig.transsion.iurl is null",
                                   includeReasons);
    } catch (const std::exception & exc) {
        result.setIncompatible
        (string("providerConfig.transsion.iurl parsing error: ")
         + exc.what(), includeReasons);
        return result;
    }
...
void getAttr(ExchangeConnector::ExchangeCompatibility & result,
    ...
    try {
        if (!config.isMember(fieldName)) {
            result.setIncompatible
            ("creative[].providerConfig.transsion." + string(fieldName)
        ...
        result.setIncompatible("creative[].providerConfig.transsion."
...
getCreativeCompatibility(const Creative & creative,
    ...
    const Json::Value & pconf = creative.providerConfig["transsion"];
    ...
    // 1.  Must have transsion.attr containing creative attributes.  These
    //     turn into TranssionCreativeAttribute filters.
    ...
    // 2. Must have transsion.type containing attribute type.
    ...
    // 3. Must have transsion.cat containing attribute type.
		  ...
		  const std::string & payload)
  {
    ...
    std::cout<<"bidrequest in transsion "<<payload<<std::endl;    
	...
	std::cout<<"transsion 400 Error UNSUPPORTED_CONTENT_TYPE"<<std::endl;
	return none;
      }
    }
    else {
      connection.sendErrorResponse("MISSING_CONTENT_TYPE_HEADER", "The request is missing the 'Content-Type' header");
      std::cout<<"transsion 400 Error MISSING_CONTENT_TYPE_HEADER"<<std::endl;
      return none;
    }
    ...
    if (it == header.headers.end()) {
      ...
      std::cout<<"transsion 400 Error MISSING_OPENRTB_HEADER"<<std::endl;
      return none;
    }
    
    // Check that it's version 2.5
    ...
    if(payload.empty()) {
      ...
      std::cout<<"transsion 400 Error EMPTY_BID_REQUEST"<<std::endl;
      return none;
    }
	...
	if(!result->auctionId.notNull()) {
	  std::cout<<"transsion 400 Error MISSING_ID"<<std::endl;
...
struct AtInit {
    AtInit() {
        ExchangeConnector::registerFactory<TranssionExchangeConnector>();
    }
} atInit;
}
