// geniee_exchange_connector.cc


#include "geniee_exchange_connector.h"
...
namespace {
	Logging::Category genieeExchangeConnectorTrace("Geniee Exchange Connector");
	Logging::Category genieeExchangeConnectorError("[ERROR] Geniee Exchange Connector", genieeExchangeConnectorTrace);
}
 
namespace RTBKIT  { 

/*****************************************************************************/
/* GENIEE EXCHANGE CONNECTOR                                                 */
/*****************************************************************************/

	std::string GenieeExchangeConnector::nobid = "nobid";

	GenieeExchangeConnector::
	GenieeExchangeConnector(ServiceBase & owner, const std::string & name)
	...
	GenieeExchangeConnector::
	GenieeExchangeConnector(const std::string & name,
					...
					const std::string & payload)
	{
		...
		size_t found = header.queryParams.uriEscaped().find(GenieeExchangeConnector::nobid);
...
//		OpenRTBExchangeConnector::getAudienceId(result);
	...
	getCampaignCompatibility(const AgentConfig & config,
		...
		const Json::Value & pconf = config.providerConfig["geniee"];
 
		try {
			cpinfo->seat = Id(pconf["seat"].asString());
			if (!cpinfo->seat)
				result.setIncompatible("providerConfig.geniee.seat is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.geniee.seat parsing error: ")
		...
		try {
			...
			if (!cpinfo->iurl.size())
				result.setIncompatible("providerConfig.geniee.iurl is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.geniee.iurl parsing error: ")
				 + exc.what(), includeReasons);
			return result;
		}
...
/** Given a configuration field, convert it to the appropriate JSON */
		...
		void getAttr(ExchangeConnector::ExchangeCompatibility & result,
			...
			try {
				if (!config.isMember(fieldName)) {
					result.setIncompatible
						("creative[].providerConfig.geniee." + string(fieldName)
				...
				result.setIncompatible("creative[].providerConfig.geniee."
	...
	getCreativeCompatibility(const Creative & creative, bool includeReasons) const {
		...
		const Json::Value & pconf = creative.providerConfig["geniee"];
...
namespace {
	...
	struct Init {
		Init() {
			ExchangeConnector::registerFactory<GenieeExchangeConnector>();
		}
	} init;
}
