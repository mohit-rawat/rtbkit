/* adcolony_exchange_connector.cc
   Matthieu Labour, December 2014
   Copyright (c) 2014 Datacratic.  All rights reserved. */


#include "adcolony_exchange_connector.h"
...
namespace {
	Logging::Category adcolonyExchangeConnectorTrace("Adcolony Exchange Connector");
	Logging::Category adcolonyExchangeConnectorError("[ERROR] Adcolony Exchange Connector", adcolonyExchangeConnectorTrace);
}
...
/*****************************************************************************/
	...
	std::string AdcolonyExchangeConnector::nobid = "nobid";

	AdcolonyExchangeConnector::
					...
					const std::string & payload)
	{
		...
		size_t found = header.queryParams.uriEscaped().find(AdcolonyExchangeConnector::nobid);
		if (found != string::npos) {
			connection.dropAuction("nobid");
			return none;
		}   
		//	  std::cout<<"adcolony check 2"<<std::endl;
		...
		if (openRtbVersion != "2.3") {
			...
			//	  std::cout<<"adcolony check 5"<<std::endl;
			return none;
		}
		...
		if(payload.empty()) {
			...
			//	  std::cout<<"adcolony check 6"<<std::endl;
			return none;
		}
...
//replacing carriername from aerospike
		...
		//		cout<<"Bidrequest after parsing in adcolony exchange connector : "<<result->toJson()<<endl;
		return result;
	}
	...
	getCampaignCompatibility(const AgentConfig & config,
		...
		const Json::Value & pconf = config.providerConfig["adcolony"];
 
		try {
			cpinfo->seat = Id(pconf["seat"].asString());
			if (!cpinfo->seat)
				result.setIncompatible("providerConfig.adcolony.seat is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.adcolony.seat parsing error: ")
		...
		try {
			...
			if (!cpinfo->iurl.size())
				result.setIncompatible("providerConfig.adcolony.iurl is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.adcolony.iurl parsing error: ")
				 + exc.what(), includeReasons);
			return result;
		}
...
/** Given a configuration field, convert it to the appropriate JSON */
		...
		void getAttr(ExchangeConnector::ExchangeCompatibility & result,
			...
			try {
				if (!config.isMember(fieldName)) {
					result.setIncompatible
						("creative[].providerConfig.adcolony." + string(fieldName)
				...
				result.setIncompatible("creative[].providerConfig.adcolony."
	...
	getCreativeCompatibility(const Creative & creative, bool includeReasons) const {
		...
		const Json::Value & pconf = creative.providerConfig["adcolony"];
...
namespace {
	...
	struct Init {
		Init() {
			ExchangeConnector::registerFactory<AdcolonyExchangeConnector>();
		}
	} init;
}
