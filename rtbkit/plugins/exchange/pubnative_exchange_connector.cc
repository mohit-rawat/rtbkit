/* pubnative_exchange_connector.cc
   Jeremy Barnes, 15 March 2013

   Implementation of the PubNative exchange connector.
*/
#include <algorithm>
#include <boost/tokenizer.hpp>

#include "pubnative_exchange_connector.h"
...
namespace {
	Logging::Category pubnativeExchangeConnectorTrace("PubNative Exchange Connector");
	Logging::Category pubnativeExchangeConnectorError("[ERROR] PubNative Exchange Connector", pubnativeExchangeConnectorTrace);
}

namespace RTBKIT {

/*****************************************************************************/
/* PUBNATIVE EXCHANGE CONNECTOR                                              */
/*****************************************************************************/

	PubNativeExchangeConnector::
	PubNativeExchangeConnector(ServiceBase & owner, const std::string & name)
	...
	PubNativeExchangeConnector::
	PubNativeExchangeConnector(const std::string & name,
	...
	getCampaignCompatibility(const AgentConfig & config,
		...
		const Json::Value & pconf = config.providerConfig["pubnative"];
 
		try {
			cpinfo->seat = Id(pconf["seat"].asString());
			if (!cpinfo->seat)
				result.setIncompatible("providerConfig.pubnative.seat is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.pubnative.seat parsing error: ")
		...
		try {
			...
			if (!cpinfo->iurl.size())
				result.setIncompatible("providerConfig.pubnative.iurl is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.pubnative.iurl parsing error: ")
				 + exc.what(), includeReasons);
			return result;
		}
...
/** Given a configuration field, convert it to the appropriate JSON */
		...
		void getAttr(ExchangeConnector::ExchangeCompatibility & result,
			...
			try {
				if (!config.isMember(fieldName)) {
					result.setIncompatible
						("creative[].providerConfig.pubnative." + string(fieldName)
				...
				result.setIncompatible("creative[].providerConfig.pubnative."
	...
	getCreativeCompatibility(const Creative & creative,
		...
		const Json::Value & pconf = creative.providerConfig["pubnative"];
...
*/		
		...
		// TODO Check with PubNative if they send the x-openrtb-version header
		...
		// get restrictions enforced by PubNative.
		//1) blocked category
...
namespace {
	...
	struct AtInit {
		AtInit() {
			ExchangeConnector::registerFactory<PubNativeExchangeConnector>();
		}
	} atInit;
}
