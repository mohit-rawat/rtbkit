/* mobfox_exchange_connector.cc
   Matthieu Labour, December 2014
   Copyright (c) 2014 Datacratic.  All rights reserved. */


#include "mobfox_exchange_connector.h"
...
namespace {
	Logging::Category mobfoxExchangeConnectorTrace("Mobfox Exchange Connector");
	Logging::Category mobfoxExchangeConnectorError("[ERROR] Mobfox Exchange Connector", mobfoxExchangeConnectorTrace);
}
...
/*****************************************************************************/
	...
	std::string MobfoxExchangeConnector::nobid = "nobid";

	MobfoxExchangeConnector::
					...
					const std::string & payload)
	{
		...
		size_t found = header.queryParams.uriEscaped().find(MobfoxExchangeConnector::nobid);
...
//		OpenRTBExchangeConnector::getAudienceId(result);
	...
	getCampaignCompatibility(const AgentConfig & config,
		...
		const Json::Value & pconf = config.providerConfig["mobfox"];
 
		try {
			cpinfo->seat = Id(pconf["seat"].asString());
			if (!cpinfo->seat)
				result.setIncompatible("providerConfig.mobfox.seat is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.mobfox.seat parsing error: ")
		...
		try {
			...
			if (!cpinfo->iurl.size())
				result.setIncompatible("providerConfig.mobfox.iurl is null",
									   includeReasons);
		} catch (const std::exception & exc) {
			result.setIncompatible
				(string("providerConfig.mobfox.iurl parsing error: ")
				 + exc.what(), includeReasons);
			return result;
		}
...
/** Given a configuration field, convert it to the appropriate JSON */
		...
		void getAttr(ExchangeConnector::ExchangeCompatibility & result,
			...
			try {
				if (!config.isMember(fieldName)) {
					result.setIncompatible
						("creative[].providerConfig.mobfox." + string(fieldName)
				...
				result.setIncompatible("creative[].providerConfig.mobfox."
	...
	getCreativeCompatibility(const Creative & creative, bool includeReasons) const {
		...
		const Json::Value & pconf = creative.providerConfig["mobfox"];
...
namespace {
	...
	struct Init {
		Init() {
			ExchangeConnector::registerFactory<MobfoxExchangeConnector>();
		}
	} init;
}
